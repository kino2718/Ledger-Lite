...
| `YearClosing` | 年度の締め(決算確定)。1レコード = 締めた1年 |
...
> 管理は行わない。会計年度末の決算確定(期間の締め・ロック)は、仕訳単位ではなく
> 会計期間単位の仕組み(`YearClosing`)で管理する。
...
年度の締め(決算確定)。1レコード = 締めた1年。会計期間は暦年(1/1〜12/31)固定。
「決算確定は仕訳単位ではなく会計期間単位で管理する」方針の実現。

| カラム | 型 | 制約 | 説明 |
| --- | --- | --- | --- |
| `id` | INTEGER | PK | 主キー |
| `userId` | INTEGER | FK → `User.id`, NOT NULL | 所有ユーザー |
| `year` | INTEGER | NOT NULL | 締めた年(例: 2025) |
| `openingEntryId` | INTEGER | FK → `JournalEntry.id`, UNIQUE, NOT NULL | 締めで生成した翌年 1/1 付の繰越仕訳 |
| `createdAt` | TIMESTAMP | NOT NULL | 締めた日時 |

#### 一意制約

- `(userId, year)` を UNIQUE。同じ年の二重締めを禁止する。
- `openingEntryId` を UNIQUE。1つの仕訳が複数の締めから参照されることはない。

#### 繰越仕訳

- 年 `Y` を締めると、翌年 `Y+1-01-01` 付・摘要「前期繰越」の仕訳を1本生成し、
  `openingEntryId` で参照する。繰越仕訳の特定・保護はこの FK 参照で行い、
  `JournalEntry` 側にフラグ列は持たない。
- 資産・負債(と元入金以外の純資産)は年末残高をそのままの向きで繰り越す
  (補助科目の内訳も保持する)。
- 新しい元入金 ＝ 元入金 ＋ 事業主借 − 事業主貸 ＋ 当年損益。
  事業主貸・事業主借・収益・費用は行にしない(翌年ゼロスタート)。
- 元入金・事業主貸・事業主借は `equity` の科目から名前で自動判定する
  (元入金は必須。無ければ締めはエラー)。
- 全残高が 0 なら明細 0 行の繰越仕訳になる(締めの記録としては有効)。

#### 締め・解除の順序

- 締めは古い年から順にのみ行える(帳簿の最初の年を除き、前年の締めが必要)。
  今年以降の年は締められない。
- 解除は最新の締め年のみ。解除すると `YearClosing` と繰越仕訳を
  1 トランザクションで削除する。

#### 集計範囲のルール

繰越仕訳は「それ以前の全履歴のスナップショット」なので、繰越仕訳より前の
明細まで合算すると二重計上になる。累計の集計(残高・貸借対照表など)は
「対象範囲より前で最後に締めた年の翌年 1/1」(＝繰越仕訳の日付)から開始する
(`lib/closing/queries.ts` の `getAggregationStart`)。締めが無ければ帳簿の
先頭から集計してよい。

#### 締め済みの年のロック

最後に締めた年以前の取引日を持つ仕訳は、作成・更新・削除できない
(更新は変更前後どちらの取引日も対象。仕訳の書き込み入口で検証する)。
繰越仕訳自体の編集・削除も拒否し、締め解除でのみ消える。
...
JournalEntry (1) ── (1) YearClosing   (繰越仕訳のみ)
```
...
> `YearClosing` も `(userId, year)` と `openingEntryId` の UNIQUE で
> 検索が賄えるため追加のインデックスは設けない。
...
- **削除時の挙動(`onDelete`)**: 外部キーごとに明示する。
  - マスタ(`Account` / `SubAccount`)は原則削除せず `isActive=false` で無効化する。
    ただし未使用(仕訳の明細がなく、勘定科目の場合は補助科目もない)のマスタは
    管理画面(`/accounts`)から物理削除できる。使用中の削除は下記の `Restrict` と
    アプリ側の事前チェックの両方で拒否される。
  ...
  - `YearClosing` → `JournalEntry`: `Restrict`。繰越仕訳は通常の仕訳削除から守り、
    締め解除でのみ削除する(締め → 仕訳の順で消す)。
  - `User` → `Account` / `JournalEntry` / `YearClosing`: `Restrict`。配下データを
    持つユーザーは削除させず、誤操作によるデータ消失を防ぐ。
...
- **仕訳単位の確定状態は持たない**: 入力した仕訳はすべて記帳済みの実データと
  ...
  (会計期間の締め・ロック)は、仕訳単位ではなく会計期間単位の仕組み
  (`YearClosing`)で管理する。
