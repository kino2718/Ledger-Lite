...
勘定科目・補助科目の管理、仕訳の記録、ダッシュボードでの残高・損益の集計
表示を行います。ローカル環境での個人利用を想定しています
...
本アプリは現在開発中です。データの土台と表示部分から実装を進めており、
現時点ではホーム画面（ダッシュボード）の表示までを実装しています。
...
- 仕訳の入力・編集画面（ダッシュボードの「新規仕訳」は現在は導線のみ）
- 帳簿・レポート（試算表の拡充、総勘定元帳など）

> 現状、仕訳データの登録は seed スクリプト経由のみです。
