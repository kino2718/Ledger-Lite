...
- 仕訳入力フォーム（借方・貸方を複数行で入力。貸借一致や科目の整合を検証して保存。
  保存後もフォームに留まり、取引日を保ったまま続けて入力できる。スマホ対応）
...
**仕訳入力** — 借方・貸方を複数行で入力し、貸借一致や科目の整合を検証して保存します。保存後もこの画面に留まるので、複数の仕訳を続けて入力できます。

![仕訳入力](./docs/screenshot-journal-new.png)

**仕訳の編集** — 既存の仕訳の内容を修正・削除できます。「コピーして新規作成」で、その仕訳を雛形に今日の日付の新しい仕訳を作ることもできます。

![仕訳の編集](./docs/screenshot-journal-edit.png)
...
**総勘定元帳（印刷用）** — 仕訳のある全科目の元帳を 1 ページに並べます。「印刷 / PDF 保存」ボタンからブラウザの印刷ダイアログを開くと、科目ごとに改ページしながら A4 縦で印刷（PDF 保存）できます。仕訳帳・試算表・損益計算書・貸借対照表にも同じ印刷ボタンがあります。

![総勘定元帳（印刷用）](./docs/screenshot-ledger-print.png)
...
主要な機能は一通り実装済みです。勘定科目・補助科目の管理・
ダッシュボード表示・仕訳の入力・一覧・編集・削除・総勘定元帳・試算表・
損益計算書・貸借対照表・年度締め（決算）・帳簿の印刷（PDF 保存）が
使えます。
...
- 仕訳の入力フォーム（`/journal/new`。貸借一致・金額・科目の整合などを検証して保存。
  保存後もフォームに留まり、取引日を保ったまま連続入力できる）
- 仕訳の一覧・編集・削除（`/journal`。仕訳帳スタイルで一覧表示し、各仕訳を編集・削除。
  編集ページの「コピーして新規作成」で、既存の仕訳を雛形に今日の日付の仕訳を作成可能）
...
- 帳簿の印刷・PDF 保存（仕訳帳・試算表・損益計算書・貸借対照表の各ページと、
  全科目の元帳を並べた印刷用ページ `/ledger/print`。各ページの
  「印刷 / PDF 保存」ボタンでブラウザの印刷ダイアログを開き、送信先で
  「PDF に保存」を選ぶと PDF になる。A4 縦を前提にレイアウトし、
  印刷時はナビゲーション等を自動で隠す）
...
- Vitest: テスト（純粋ロジックの単体テスト＋実 DB を使う統合テスト＋UI コンポーネントのテスト）
- Testing Library / jsdom: UI コンポーネントのテスト（ブラウザなしでフォームの挙動を検証）
...
（`dev.db` には影響しません）。また、Testing Library ＋ jsdom による
UI コンポーネントのテストも含みます。
